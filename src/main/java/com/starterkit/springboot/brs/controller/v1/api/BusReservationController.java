...
public class BusReservationController {
    ...
    public Response getAllHistory() {
        return Response
                .ok()
                .setPayload(busReservationService.getAllHistory());
    }
        ...
        if (userDto.isPresent()) {
            ...
            if (tripDto.isPresent()) {
                ...
                if (tripScheduleDto.isPresent()) {
                    ...
                    Optional<HistoryDto> historyDto = Optional
                            .ofNullable(busReservationService.history(tripScheduleDto.get(), userDto.get()));
                    if (ticketDto.isPresent()) {
                    	Response.ok().setPayload(historyDto.get());
